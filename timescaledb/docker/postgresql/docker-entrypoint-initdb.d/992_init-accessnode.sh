...
until PGPASSWORD=$POSTGRES_PASSWORD psql -h timescaledb-dn01 -d "$POSTGRES_DB" -U "$POSTGRES_USER" -c '\q'; do
    sleep 5s
done
until PGPASSWORD=$POSTGRES_PASSWORD psql -h timescaledb-dn02 -d "$POSTGRES_DB" -U "$POSTGRES_USER" -c '\q'; do
    sleep 5s
done
